...
import {
  Users, Shield, BarChart3, FileText, CheckCircle, XCircle,
  AlertTriangle, TrendingUp, PieChart, UserCog, Clock
} from "lucide-react";
...
const FacultyDashboard = () => {
  ...
  const { data: paperStats, isLoading: statsLoading } = useQuery({
    queryKey: ["faculty-paper-stats"],
    enabled: !!user,
    queryFn: async () => {
      ...
      return { total: total || 0, approved: approved || 0, pending: pending || 0, rejected: rejected || 0, users: userCount || 0 };
    },
  });

  const isDataLoading = profilesLoading || rolesLoading || papersLoading || statsLoading;
  ...
  if (authLoading) return <Layout><LoadingOverlay visible={true} message="Loading..." /></Layout>;
  if (!user) return <Navigate to="/login" />;
  if (!hasRole("faculty") && !hasRole("admin")) return (
    <Layout>
      <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4">
        <Shield className="h-12 w-12 text-destructive mb-4" />
        <h2 className="text-xl font-display font-bold text-foreground mb-2">Access Denied</h2>
        <p className="text-muted-foreground text-sm">You need faculty privileges to access this dashboard.</p>
      </div>
    </Layout>
  );
  ...
  return (
    <Layout>
      <LoadingOverlay visible={isDataLoading} message="Loading dashboard..." />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-accent to-accent/70 shadow-lg shadow-accent/20">
              <Shield className="h-5 w-5 text-accent-foreground" />
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-display font-extrabold tracking-tight">
                Faculty <span className="gradient-text">Dashboard</span>
              </h1>
              <p className="text-sm text-muted-foreground">Manage papers, view users, and analytics</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          ...
          <StatCard icon={FileText} label="Total Papers" value={String(paperStats?.total || 0)} delay={0.05} />
          <StatCard icon={CheckCircle} label="Approved" value={String(paperStats?.approved || 0)} delay={0.1} accentColor="bg-gradient-to-br from-emerald-500 to-teal-500" />
          <StatCard icon={AlertTriangle} label="Pending" value={String(paperStats?.pending || 0)} delay={0.15} accentColor="bg-gradient-to-br from-amber-500 to-orange-500" />
          <StatCard icon={XCircle} label="Rejected" value={String(paperStats?.rejected || 0)} delay={0.2} accentColor="bg-gradient-to-br from-red-500 to-pink-500" />
        </div>
